...
public class CourseServiceImpl extends ServiceImpl<CourseMapper, Course> implements CourseService {
    ...
    public String saveCourseInfo(CourseInfoForm courseInfoForm) {
        ...
        subject = Optional.ofNullable(subject).orElseThrow(() -> GuiException.from(ResultCode.DATA_NO_EXIST));
        ...
        if(!saveResult){
            //添加失败，提示用户
            throw GuiException.from(ResultCode.SAVE_ERROR);
        }
    ...
    public void updateCourse(CourseInfoForm courseInfoForm) {
        ...
        if(!updateResult){
            throw GuiException.from(ResultCode.UPDATE_ERROR);
        }
